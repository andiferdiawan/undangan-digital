 ...
 */
export async function sendBrandEmail(event: H3Event, to: string, subject: string, email: BrandEmail): Promise<{ sent: boolean, reason?: string }> {
  const cfg = useRuntimeConfig(event).email as { resendApiKey: string, from: string }
  ...
  try {
    await $fetch('https://api.resend.com/emails', {
      ...
      body: { from: cfg.from, to: [to], subject, html: brandEmailHtml(email), text: brandEmailText(email) },
      timeout: 10000,
    })
    return { sent: true }
  }
